                ...
                <div class="content-header-left col-md-6 col-12 mb-2 breadcrumb-new">
                    ...
                    <div class="row breadcrumbs-top d-inline-block">
                        <div class="breadcrumb-wrapper col-12">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item"><a href="#">{{ __('keywords.dashboard') }}</a>
                                </li>
            ...
            <div class="content-body">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title" id="basic-layout-colored-form-control">{{ __('keywords.roles') }} </h4>
                    ...
                    <div class="card-content">
                        <div class="card-body">
                            ...
                            <a href="{{ route('admin.roles.create') }}" class="btn btn-primary">{{ __('keywords.add') }}</a><br><br>
                            <table class="table table-responsive-sm">
                                <thead>
                                    <tr>
                                        <th scope="col">#</th>
                                        <th scope="col">{{ __('keywords.role_name') }}</th>
                                        <th scope="col">{{ __('keywords.permissions') }} </th>
                                            ...
                                            <td>
                                                ...
                                                @if (config('app.locale') == 'ar')
                                                    @foreach ($role->permission as $perm)
                                                        {{-- if key in permission config == perm in role tables , print the value of permission --}}
                                                ...
                                                @else
                                                    @foreach ($role->permission as $perm)
                                                    {{--  متظهرش الادمن نفسه عشان تظهر المهمه بتاع الادمن واليوزر وكدا  --}}
                                                        @foreach (Config::get('permessions') as $key => $value)
                                                            {{ $key == $perm ? $value . ' , ' : '' }}
                                                        @endforeach
                                                    @endforeach
                                                @endif
                                            ...
                                            <td>
                                                <div class="dropdown float-md-left">
                                                    <button class="btn btn-danger dropdown-toggle round btn-glow px-2"
                                                        ...
                                                        aria-haspopup="true"
                                                        aria-expanded="false">{{ __('keywords.operations') }}</button>
                                                    <div class="dropdown-menu" aria-labelledby="dropdownBreadcrumbButton">
                                                        <a class="dropdown-item"
                                                            href="{{ route('admin.roles.edit', $role->id) }}"><i
                                                                class="la la-edit"></i>{{ __('keywords.edit') }}</a>
                                                        ...
                                                        <div class="dropdown-divider"></div><a class="dropdown-item"
                                                            ...
                                                            onclick="if(confirm('Are you sure you want to delete this role?')){document.getElementById('delete-form-{{ $role->id }}').submit();} return false"><i
                                                                class="la la-trash"></i> {{ __('keywords.delete') }}</a>
                                                    </div>
                                                </div>
                                            </td>
                                        </tr>
